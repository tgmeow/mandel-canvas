...
func createDefaultImage(imageWidth int, imageHeight int) image.Image {
	xCenter, yCenter := -0.5, 0.0
	scale := 1.0
	maxIter := 100
	nThreads := 1
	cb := getComputeBounds(xCenter, yCenter, scale, maxIter, imageWidth, imageHeight)
	ib := ImageBounds{xMax: imageWidth, yMax: imageHeight}
	myImage := image.NewNRGBA(image.Rect(0, 0, imageWidth, imageHeight))
	computeImage(myImage, &cb, &ib, nThreads)
	return myImage
}
